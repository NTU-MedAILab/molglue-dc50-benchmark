#!/usr/bin/env python3
"""Render the four publication figures from redistribution-safe source data."""
...
FULL_GRAPH = "full_context_graph_mpn"
CHEM_GRAPH = "chemistry_graph_mpn"
...
def save_bundle(fig: plt.Figure, output_dir: Path, stem: str) -> None:
    ...
    ensure_svg_font_family(svg_path)
    ...
    # Matplotlib's TIFF writer may retain an opaque alpha channel. Flatten it
    # explicitly so that journal ingestion receives a conventional RGB TIFF.
...
def figure1(source_dir: Path, output_dir: Path) -> None:
    ...
    repeat_summary = pd.read_csv(
        source_dir / "source_data_repeat_summary.csv"
    )
    if len(repeat_summary) != 1:
        raise ValueError("source_data_repeat_summary.csv must contain one aggregate row")
    ...
    panel_label(ax_d, "d")
    repeated_compounds = int(repeat_summary["n_repeated_compounds"])
    repeated_rows = int(repeat_summary["n_rows_in_repeated_compounds"])
    wide_range = int(repeat_summary["n_compounds_pdc50_range_ge_1"])
    repeated_contexts = int(repeat_summary["n_repeated_exact_context_groups"])
    ax_d.text(
        0.985,
        0.055,
        (
            f"{repeated_compounds}/{int(repeat_summary['n_compounds']):,} compounds repeated "
            f"({repeated_rows}/{int(repeat_summary['n_total_rows']):,} rows)\n"
            f"{wide_range} compounds span ≥1 pDC50 unit\n"
            f"{repeated_contexts}/{int(repeat_summary['n_exact_context_groups']):,} "
...
def figure2(source_dir: Path, output_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(
        2,
        2,
        figsize=(5.77, 5.1969 * JOURNAL_SCALE),
    ...
    rows.append(
        {
            "label": "Full − chem (graph MPN)",
            "rho": graph_internal["delta_spearman_observed"],
            "rho_low": graph_internal["delta_spearman_ci_low"],
            "rho_high": graph_internal["delta_spearman_ci_high"],
            "rmse": graph_internal["delta_rmse_observed"],
            "rmse_low": graph_internal["delta_rmse_ci_low"],
            "rmse_high": graph_internal["delta_rmse_ci_high"],
            "identity": "post_hoc_graph",
        }
    )
...
def figure3(source_dir: Path, output_dir: Path) -> None:
    ...
    for frame, regime, identity in (
        ...
        for row in frame.to_dict("records"):
            contrast_rows.append(
                {
                    "label": (
                        f"{'Src' if row['protocol'] == 'source_ood' else 'Tgt'} · "
    ...
    graph_contrasts = graph[
        (graph["record_type"] == "contrast")
        & (graph["split_regime"] != "internal")
    ]
    for row in graph_contrasts.to_dict("records"):
        contrast_rows.append(
            {
                "label": (
                    f"{'Src' if row['protocol'] == 'source_ood' else 'Tgt'} · "
                    f"graph {'strict' if row['split_regime'].startswith('strict') else 'cold'}"
                ),
                "rho": row["delta_domain_macro_spearman_observed"],
                "rho_low": row["delta_domain_macro_spearman_ci_low"],
                "rho_high": row["delta_domain_macro_spearman_ci_high"],
    ...
    for axis, metric, low, high, xlabel in (
        ...
        axis.tick_params(axis="y", labelsize=5.5, pad=2)
...
def figure4(source_dir: Path, output_dir: Path) -> None:
    ...
    fig = plt.figure(
        figsize=(5.77, 5.3150 * JOURNAL_SCALE)
    )
    grid = fig.add_gridspec(
        2,
        2,
    ...
    supported = (
        ...
        "• matched feature-block gains\n"
        "• modest, heterogeneous transfer\n"
...
def write_qa_notes(source_dir: Path, output_dir: Path) -> None:
    required = [
        ...
        "source_data_repeat_summary.csv",
        ...
        "source_data_graph_model_family_sensitivity.csv",
    ]
    notes = [
        "# Figure QA notes",
        "",
        ...
        "",
        "| Figure | SVG width (mm) | Width ≤170 mm | PNG preview | TIFF delivery |",
        "|---|---:|:---:|---:|---:|",
    ]
    for stem in (
        ...
        svg_text = (output_dir / f"{stem}.svg").read_text(encoding="utf-8")
        width_token = svg_text.split('width="', 1)[1].split('"', 1)[0]
        if not width_token.endswith("pt"):
            raise ValueError(f"Unexpected SVG width unit: {width_token}")
        width_mm = float(width_token[:-2]) * 25.4 / 72
        with Image.open(output_dir / f"{stem}.png") as png:
            png_size = f"{png.width}×{png.height} px at 300 dpi"
        with Image.open(output_dir / f"{stem}.tiff") as tiff:
            tiff_size = (
                f"{tiff.width}×{tiff.height} px at 600 dpi, {tiff.mode}"
            )
        notes.append(
            f"| {stem} | {width_mm:.1f} | "
            f"{'PASS' if width_mm <= 170 else 'FAIL'} | "
            f"{png_size} | {tiff_size} |"
        )
    notes.extend(
        [
            "",
            "SVG files retain text nodes; PDF files use embedded TrueType "
            "sans-serif fonts.",
            "",
            "## Manual visual review of the v1 release",
            "",
            "- PASS (2026-07-29): all four PNG previews were inspected at "
            "full resolution; no panel, title, axis label, tick label, "
            "legend, interval or annotation is cropped.",
            "- PASS: Fig. 4a's legend occupies the data-free upper-left "
            "region and does not obscure any source-domain point.",
            "- PASS: full versus chemistry uses filled versus open markers; "
            "post-hoc HGB uses square markers and the molecular-graph "
            "sensitivity uses diamond markers; leave-one-domain-"
            "out sign is encoded by both bar direction and hue; OOD coverage "
            "uses high-luminance contrast.",
            "",
            "## Figure-to-source-data map",
            "",
            "- Fig. 1: `provenance_by_source.csv`, "
            "`provenance_by_target.csv`, `data_missingness.csv`, "
            "`source_data_repeat_summary.csv` (redistribution-safe aggregate).",
            "- Fig. 2: `source_data_internal_models.csv`, "
            "`source_data_internal_contrasts.csv`, "
            "`source_data_hgb_model_family_sensitivity.csv`, "
            "`source_data_graph_model_family_sensitivity.csv`.",
            "- Fig. 3: `source_data_confirmatory_ood.csv`, "
            "`source_data_strict_ood.csv`, "
            "`source_data_hgb_model_family_sensitivity.csv`, "
            "`source_data_graph_model_family_sensitivity.csv`.",
            "- Fig. 4: `source_data_ood_domains.csv`, "
            "`source_data_lodo.csv`, `source_data_calibration.csv`.",
            "",
        ]
    )
    notes.extend(
